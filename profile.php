...
if ($user_id!=$id) {
  ...
  if ($block->is_you_block($user->getID($id),$user_name)===true) {
    header("Location:block.php?name=$id");
  }else {
    # code...
    if ($user->getID($id)!=$user_id ) {
      # code...
      $visitor->visit($user->getID($id),$user_id,$user_name);
    }
...
<div id="page">
  ...
  <span class="w3-opennav w3-xlarge w3-hide-large" onclick="w3_open()">&#9776;</span>
    ...
    <div class="w3-col s12" id="content">
      ...
      <?php
        $photo = new Photos();
        $photo->photoProfile($user->getID($id));
       ?>
     ...
     <ul class="info">
       <h6>Vartotojo informacija</h6>
        ...
        if ($id==$user_name) {
          echo '<input type="button" value="Redaguoti profilį"/ id="profile_btn"></br>
          <input type="button" value="Trinti profilį" id="profile_dlt"/>';
        }else {
                if ($block->is_block($user_id,$id)===true) {
                  echo '<input type="button" value="Rašyti žinutę"/ id="send_msg_btn"></br>
                        <input type="button" value="Kviesti draugauti" id="friends_query_dlt"/></br>
                        <input type="button" value="Atblokuoti" id="unblock_member"/>';
                }else {
                  echo '<input type="button" value="Rašyti žinutę"/ id="send_msg_btn"></br>
                        <input type="button" value="Kviesti draugauti" id="friends_query_dlt"/></br>
                        <input type="button" value="Užblokuoti" id="block_member"/>';
                }
        }
...
<div id="modal_send_msg" class="w3-modal">
 <div class="w3-modal-content w3-animate-zoom">
   <header class="w3-container w3-teal">
     <span onclick="document.getElementById('modal_send_msg').style.display='none'" class="w3-closebtn">&times;</span>
     Siųst žinutę
   </header>
   <div class="w3-container">
     <form class="w3-col s12">
       <textarea placeholder="Jūsų žinutė" name="new_msg"></textarea>
       <input type="button" id="new_msg_btn1" value="Siųsti"/>
